...
const MenuEditPage = () => {
  ...
  type FormValues = z.infer<typeof MenuEditSchema>;
  ...
  useEffect(() => {
    if (menu) {
      reset({
        ...
        dishList: menu.dishList.map((dish) => ({
          dishId: dish.dishId,
  ...
  if (!menu) {
    return (
      <main className="mx-auto min-h-[calc(100svh-64px)] w-full max-w-4xl p-6">
  ...
  const onSubmit = (data: FormValues) => {
    ...
    toast.success("Menü başarıyla güncellendi!", {
      description: `"${data.menuName}" menüsü düzenlendi.`,
    });

    router.push(`/menu/detail/${menu.id}`);
  };
        ...
        <div className="mb-8 flex items-center justify-between">
          <div className="flex items-center gap-3 ">
            <Button variant="outline" size="icon" asChild>
              <Link href={`/menu/detail/${menu.id}`}>
                <ArrowLeft className="h-4 w-4" />
              </Link>
            </Button>
            <div className="flex items-center gap-2">
              <ChefHat className="h-6 w-6 text-primary" />
              <h1 className="text-2xl font-bold">Menüyü Düzenle</h1>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-8">
          ...
          <div className="sticky bottom-0 w-full flex items-center justify-end gap-3 rounded-xl border bg-card p-4 shadow-lg">
            <Button
              type="button"
              variant="outline"
              onClick={() => router.push(`/menu/detail/${menu.id}`)}
              className="md:flex-1"
            >
              İptal
            </Button>
